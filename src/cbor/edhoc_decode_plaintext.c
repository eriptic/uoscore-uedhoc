		...
		zcbor_state_t *state, struct map_kid_ *result)
{
	...
	&& (zcbor_union_start_code(state) && (int_res = ((((zcbor_int32_decode(state, (&(*result)._map_kid_int)))) && (((*result)._map_kid_choice = _map_kid_int) || 1))
	|| (((zcbor_bstr_decode(state, (&(*result)._map_kid_bstr)))) && (((*result)._map_kid_choice = _map_kid_bstr) || 1))), zcbor_union_end_code(state), int_res))));
