...
static bool decode_plaintext2(zcbor_state_t *state, struct plaintext2 *result);
...
static bool decode_plaintext2(
		zcbor_state_t *state, struct plaintext2 *result)
{
	zcbor_log("%s\r\n", __func__);
	bool int_res;

	bool tmp_result = (((((*result).plaintext2_C_R_present = ((zcbor_bstr_decode(state, (&(*result).plaintext2_C_R)))), 1)
	&& ((zcbor_union_start_code(state) && (int_res = ((((decode_map(state, (&(*result).plaintext2_ID_CRED_R_map_m)))) && (((*result).plaintext2_ID_CRED_R_choice = plaintext2_ID_CRED_R_map_m_c), true))
	|| (zcbor_union_elem_code(state) && (((zcbor_bstr_decode(state, (&(*result).plaintext2_ID_CRED_R_bstr)))) && (((*result).plaintext2_ID_CRED_R_choice = plaintext2_ID_CRED_R_bstr_c), true)))
	|| (((zcbor_int32_decode(state, (&(*result).plaintext2_ID_CRED_R_int)))) && (((*result).plaintext2_ID_CRED_R_choice = plaintext2_ID_CRED_R_int_c), true))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).plaintext2_SGN_or_MAC_2))))
	&& ((*result).plaintext2_EAD_2_present = ((zcbor_bstr_decode(state, (&(*result).plaintext2_EAD_2)))), 1))));
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_plaintext2, sizeof(states) / sizeof(zcbor_state_t), 4);
}
